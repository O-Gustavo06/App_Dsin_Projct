...
import { createVaga, listVagas } from '../../services/Api';
...
import { payParkingSession, getWallet } from '../../services/PaymentApi';
...
export default function HomeScreen({ navigation }) {
  ...
  const loadSpots = async () => {
    try {
      try {
        const remoteSpots = await listVagas();
        if (remoteSpots.length > 0) {
          setSpots(remoteSpots);
          await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(remoteSpots));
          return;
        }
      } catch (remoteError) {
        // fallback local
  ...
  useEffect(() => {
    ...
    loadHidden();
    loadBalance();
    getSessionUserId().then(setUserId).catch(() => setUserId(1));
  ...
  const createSpotAt = async (latitude, longitude) => {
    try {
      const newSpot = await createVaga({
        title: `Vaga ${spots.length + 1}`,
        description: 'Vaga criada manualmente',
        latitude,
        longitude,
      });
      ...
      setSpots(updated);
      setSelectedSpot({ id: newSpot.id, title: newSpot.title });
      try { await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(updated)); } catch (storageErr) { console.error('Erro ao salvar vaga:', storageErr); }
    }
  };
  ...
  const handlePayWithTickets = async () => {
    ...
    try {
      try {
        const apiRes = await payParkingSession(userId, fare, 'balance');
        ...
        try {
          ...
          setUserBalance(currentRemote);
          await AsyncStorage.setItem(BALANCE_KEY, String(currentRemote));
        } catch (syncErr) {
