...
import { createVeiculo, editVeiculo, listVeiculos } from '../../services/Api';
...
export default function DadosVeiculo({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        try {
          const remoteVehicles = await listVeiculos(userId);
          if (remoteVehicles.length > 0) {
            setVehicle(remoteVehicles[0]);
            await AsyncStorage.setItem(VEHICLE_KEY, JSON.stringify(remoteVehicles[0]));
          } else {
  ...
  const saveVehicle = async () => {
    ...
    try {
      ...
      const savedVehicle = form.IdVeiculo && /^\d+$/.test(String(form.IdVeiculo))
        ? await editVeiculo(form.IdVeiculo, payload)
        : await createVeiculo(payload);

      await AsyncStorage.setItem(VEHICLE_KEY, JSON.stringify(savedVehicle));
      ...
      try {
        ...
        setVehicle(form);
        setEditVisible(false);
        Alert.alert('Sucesso', 'Dados do veículo salvos localmente.');
      } catch (storageError) {
        console.error('Erro ao salvar vehicle', err, storageError);
